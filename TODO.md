...
2. [ ] Make Rust bridge signatures verifiable rather than symbol-checked.
   `extern rust "snacc_user_x" fun ...` declares parameter and result types that
   nothing validates against the Rust side; the linker only proves the symbol
   exists, so a mismatched arity or type is undefined behavior at runtime.
   Specified by [RFC 007](docs/specs/007-bridge-signature-verification.md).

3. [ ] Reject duplicate function and external-function parameter names.
...
4. [ ] Make object-cache publication safe for concurrent builds.
...
5. [ ] Use a constant-time comparison for the workbench session token.
...
6. [ ] Make the conformance runner execute every `examples/*.nrs` with a
...
7. [ ] Add workbench request-rejection tests. Every control is implemented and
...
8. [ ] Add workbench process tests: supplied stdin reaches a helper child and
...
9. [ ] Add focused `snacc-driver` tests in `crates/snacc-driver/tests/`:
...
10. [ ] Add `snacc-runtime` ABI coverage in `crates/snacc-runtime/tests/` for
    each exported `snacc_print_*` symbol and the `force_link` retention
    contract.

11. [ ] Extend `tools/package-windows.ps1` so `-IncludeDirectCompiler` compiles
...
12. [ ] `LANGUAGE.md` and `GRAMMAR.ebnf` are both empty while RFCs 005 and 006
...
13. [ ] After all acceptance criteria are verified, change RFCs 004, 005, and
